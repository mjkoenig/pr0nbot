...
Pr0nbot is a spidering bot that takes a single URL and wanders the web downloading images and videos from free porn sites. The project started as a joke that went too far, then became a fun way to improve my Python skills. Right now it's a fairly robust spidering engine which can parse some pretty broken HTML without crashig (and boy, porn sites have horrible, horrible code).
...
Linux: Use the install script provided or simply copy pr0nbot.py to somewhere on your executable path, rename it to 'pr0nbot' sans '.py' and make it executable. That's basically what the install script does anyway. 

Windows: Put the script file anywhere you like and run it from the console (python pr0nbot.py ...).
